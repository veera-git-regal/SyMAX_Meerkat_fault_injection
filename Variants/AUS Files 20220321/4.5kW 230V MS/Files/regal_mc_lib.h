...
#define default_RAMP_STEP                       2              //Current ramping steps for both ramping up and ramping down
#define default_SPEED_TRANSITION                (uint16_t) (OBS_MINIMUM_SPEED_RPM/6)    //transition speed from steady-state to ramp down
#define alpha_br                         (int16_t) (FP8*0.9)                    //LPF gain used for filtering raw speed measurements
#define BYTE_SHIFT                      8   
...
#define default_MOTOR_INERTIA           45.0 /* just for test on the blender */
#define default_MOTOR_VISCOUS_DAMPING   10.0      /* RPa: coefficient of dynamic friction */
#define TORQUE_CONSTANT                         (MOTOR_VOLTAGE_CONSTANT/ 60.459979) /* RPa: Ke/((3/sqrt(3))*(60/2/pi)/1000); Nm/A (rms) */
#define FAN_INERTIA                             3.0 /*RPa: g*m^2 */
#define TOTAL_INERTIA                           (float) ( default_MOTOR_INERTIA + FAN_INERTIA )
#define NOMINAL_DC_VOLTAGE              325 /*230V variant: 375, 460V variant: 650; nominal dc voltage as seen on the bus */
//////////////////////////////////////////////////////////
...
#define default_MAX_COMMANDABLE_SPEED   (uint16_t) (0.72 * MAX_APPLICATION_SPEED_RPM) /* maximum commandable speed is based on the maximum application speed*/
#define default_SPEED_UP_RAMP_RATE      100 /* settings for ramp up rate */
...
#define default_OVP_THRESHOLD           (int16_t) (1.06 * 4500)  /* based on mechanical power and efficiency */
...
#define default_BK_VBUS_CLIP             (uint16_t) (0.70 * ( OV_VOLTAGE_THRESHOLD_V - NOMINAL_DC_VOLTAGE))   /* based on the difference between voltage levels and OVP setting */
#define default_BK_ENDSPEED             18 //(uint16_t) (1110 / MOTOR_VOLTAGE_CONSTANT)   /* RPa: Ke, SNR, and voltage levels, arbitrary: 10V bemf level, but this needs to consider OCP */
...
#define default_STOP_TIMEOUT            (uint32_t) 80000 /* in msec, goes back to IDLE when timesout */
...
#define default_BK_RAMP_a               13//(int16_t) (IMAX_TRAJECTORY * FP8) /* RPa: motor ellipse equation, 13 */
#define default_BK_RAMP_b               -6//(int16_t) (-130 * IMAX_TRAJECTORY) /* RPa: motor ellipse equation, -6 */
#define default_BK_RAMP_c               1220//(int16_t) ((4225 * IMAX_TRAJECTORY) + MIN_CURRENT_SPD_TRANS) /* RPa: motor ellipse equation , 1220*/
...
#define default_OTF_MIN_BEMFG           (uint16_t) (0.9 * default_OTF_DBEMFG) /* RPa: gain during synchronisation stage */
#define default_OTF_MAX_SYNC_SPEED      (uint16_t) (OBS_MINIMUM_SPEED_RPM / 4 )/* RPa: Note that value is used as RPM/6 */
...
#define default_WM_MAX_REVERSE_SPEED            110//(uint16_t) (OBS_MINIMUM_SPEED_RPM / 5 ) /* above the observer min speed, raise a startup failure */
...
#define default_WM_CLAMP_CURRENT                (uint16_t) (PHASE3_FINAL_CURRENT + (0.3 * (WM_MAX_CLAMP_CURRENT - PHASE3_FINAL_CURRENT))) 
