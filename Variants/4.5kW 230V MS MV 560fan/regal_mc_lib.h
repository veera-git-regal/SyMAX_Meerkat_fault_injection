...
#define default_MOTOR_INERTIA           45 //60.0  /* just for test on the blender */
...
#define FAN_INERTIA                     20.0 /*RPa: g*m^2 */
...
#define default_SPEED_UP_RAMP_RATE      50 /* settings for ramp up rate */
#define default_SPEED_DOWN_RAMP_RATE    50 /* settings for ramp down rate */
...
#define default_CONTROLLED_BRAKING     DISABLE //ENABLE                                                     /* enable/disable for the Regal Controlled Braking feature*/
#define default_DECEL_CONTROL          DISABLE //ENABLE  /* enable/disable for the Regal Controlled Deceleration feature */
#define default_BK_CURRENTSEEDING      (int16_t)(0.03 * PHASE4_FINAL_CURRENT)                      /* maximum current seed at the start controlled braking */
#define default_BK_VBUS_CLIP           (uint16_t) (0.25 * ( OV_VOLTAGE_THRESHOLD_V - NOMINAL_DC_VOLTAGE))   /* based on the difference between voltage levels and OVP setting */
#define default_BK_ENDSPEED            20 //(uint16_t) (1110 / MOTOR_VOLTAGE_CONSTANT)   /* RPa: Ke, SNR, and voltage levels, arbitrary: 10V bemf level, but this needs to consider OCP */
...
#define default_STOP_TIMEOUT           (uint32_t) 50000 /* in msec, goes back to IDLE when timesout */
...
#define default_WM_SHORTCOIL_DURATION        (uint16_t) 3000
// RPa: set the powerup duration to 0 or small value, e.g. 100 when testing OTF and others for quicker startup when
#define default_POWERUP_DURATION        5000/* factor in other delays on the system */
